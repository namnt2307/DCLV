...
class EncounterModel(models.Model):
    ...
    encounter_reason = models.CharField(null=True, max_length=255)
    encounter_location = models.ForeignKey(
        ClinicalDepartment, on_delete=SET_NULL, null=True
    )
...
class ConditionModel(models.Model):
    ...
    condition_note = models.CharField(max_length=255, null=True, blank=True)
...
class ServiceRequestModel(models.Model):
    ...
    service_code = models.CharField(max_length=255, null=True)
    ...
    service_requester_identifier = models.ForeignKey(
        PractitionerModel,
        null=True,
        on_delete=SET_NULL,
        related_name="service_requester",
    )
    service_note = models.CharField(max_length=255, null=True, blank=True)
    ...
    service_reason_code = models.CharField(max_length=255, null=True, blank=True)
...
class ObservationModel(models.Model):
    ...
    observation_performer_identifier = models.ForeignKey(
        PractitionerModel, null=True, on_delete=models.SET_NULL
    )
    observation_value_quantity = models.CharField(default="", max_length=100, null=True)
    observation_value_unit = models.CharField(default="", max_length=100)
    observation_note = models.CharField(
        default="", max_length=300, null=True, blank=True
    )
...
class ProcedureModel(models.Model):
    ...
    procedure_reason_code = models.CharField(max_length=255, null=True)
    procedure_outcome = models.CharField(
        max_length=100, null=True, choices=PROCEDURE_OUTCOME_CHOICES
    )
    procedure_complication = models.CharField(max_length=255, null=True, blank=True)
    procedure_follow_up = models.CharField(max_length=255, null=True, blank=True)
    procedure_note = models.CharField(max_length=255, null=True, blank=True)
...
class MedicationModel(models.Model):
    ...
    medication_reason_code = models.CharField(max_length=255, default="")
    ...
    dosage_period_unit = models.CharField(
        max_length=100, choices=UNIT_CHOICES, null=True
    )
    dosage_duration = models.PositiveIntegerField(default=1)
    dosage_duration_unit = models.CharField(
        max_length=100, choices=UNIT_CHOICES, null=True
    )
    ...
    dosage_when = models.CharField(
        choices=DOSAGE_WHEN_CHOICES, max_length=100, blank=True
    )
...
def upload_to_folder(instance, filename):
    return "static/img/patient/procedure_results/" + "/".join(
        [
            instance.procedure_identifier.encounter_identifier.patient.identifier,
            instance.procedure_identifier.encounter_identifier.encounter_identifier,
            filename,
        ]
    )
...
class DiagnosticReportModel(models.Model):
    ...
    diagnostic_conclusion = models.CharField(max_length=255)
...
class DischargeDisease(models.Model):
    ...
    disease_search = models.CharField(max_length=255)
...
class Image(models.Model):
    ...
    image_category = models.CharField(max_length=255)
